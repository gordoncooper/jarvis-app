...
Slide order left→right. Arrow keys / drag. No center-chat in Breath.

| # | Room | Rendering |
| --- | --- | --- |
| 0 | Login | Full-bleed rack-room still + HTML/SVG wordmark + inverted triangle Enter. **No** username/password. |
| 1 | Breath | R3F night Earth backdrop (+ drei / postprocessing) + rim HUD + CmdBar + dialogue strip. |
...
- Real: `/health`, `/v1/session` (structured briefing preferred;
  ...
  (poll ~2s) for Breath chips + NOC.
- Do **not** scrape noc.lan, home.lan, Prometheus, or Grafana from glass.
- Confirm UI: ConfirmCard on CMD Channel; Breath uses the dialogue strip for replies (confirm
  lives in Channel / APPLY). Hands/memory confirms stay orchestrator-owned.

## Pack / build

- Product pack: `cockpit` (`JARVIS_THEME=cockpit`). The theme library and what
  a theme must provide is `../../../../docs/THEMES.md`; this file is cockpit's
  look, not the register of themes.
- Production: esbuild → static nginx. Vite only if the operator asks for bastion HMR.
- Allowed: React, three / R3F / drei / postprocessing (Earth globe only), motion, SVG,
